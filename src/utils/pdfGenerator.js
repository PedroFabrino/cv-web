...
export function generateDirectPDF(i18nData, t) {
  const doc = new jsPDF({
    unit: 'pt',
    format: 'a4',
  });
  ...
  const emailClean = i18nData.basics.email ? i18nData.basics.email.replace("{'@'}", "@") : '';
  const contactText = `${i18nData.basics.location.city}, ${i18nData.basics.location.region}  •  ${emailClean}  •  ${i18nData.basics.phone}  •  ${i18nData.basics.url}`;
  ...
  if (i18nData.basics.summary) {
    const summaryTitle = t ? t('ui.about') : 'Professional Summary';
  ...
  if (i18nData.work && i18nData.work.length) {
    const expTitle = t ? t('ui.experience') : 'Work Experience';
  ...
  if (i18nData.skills && i18nData.skills.length) {
    const skillsTitle = t ? t('ui.technicalSkills') : 'Technical Skills';
    addSectionHeading(skillsTitle);
    doc.setFontSize(9.5);
  ...
  if (i18nData.certifications && i18nData.certifications.length) {
    const certTitle = t ? t('ui.certifications') : 'Certifications';
    addSectionHeading(certTitle);
    doc.setFontSize(9.5);
  ...
  const fileName = `${i18nData.basics.name.replace(/\s+/g, '_')}_CV.pdf`;
  doc.save(fileName);
}
